...
def _build_tra(service: str) -> bytes:
    ...
    exp = (now + timedelta(minutes=10)).strftime("%Y-%m-%dT%H:%M:%S")
...
def generate_key_and_csr(cuit: str, cert_dir: str) -> tuple[Path, Path]:
    """
    Generate an RSA private key and a CSR for the given CUIT.
    ...
    # Save private key (PEM, no passphrase — user's filesystem is the security boundary)
